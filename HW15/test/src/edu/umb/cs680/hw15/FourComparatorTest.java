...
public class FourComparatorTest {
    ...
    ApfsLink y = new ApfsLink(null, "y", 0, localdatetime4, b);
        ...
        public void getChildrenAlphabeticalTest() {
            ...
            LinkedList<ApfsElement> children = home.getChildren((ApfsElement e1, ApfsElement e2) -> {
        ...
        public void getChildrenReverseAlphabeticalTest() {
            ...
            LinkedList<ApfsElement> children
                    = home.getChildren(Comparator.comparing(
                            (ApfsElement element) -> element.getName(), Comparator.reverseOrder()));
            String[] actual = new String[children.size()];
            for(int i =0; i < children.size(); i++)
                actual[i] = children.get(i).getName();
            assertArrayEquals(expected, actual);
    ...
    public void getChildrenTimestampBasedTest() {
         ...
         LinkedList<ApfsElement> children
                 = home.getChildren((ApfsElement e1, ApfsElement e2) -> e2.getLastModified().compareTo(e1.getLastModified()));
         String[] actual = new String[children.size()];
         for(int i =0; i < children.size(); i++)
             actual[i] = children.get(i).getName();
         assertArrayEquals(expected, actual);
